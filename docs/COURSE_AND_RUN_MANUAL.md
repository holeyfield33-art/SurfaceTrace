...
The current UI supports HAR import, inventory counts, endpoint focus, raw redacted HTTP inspection, parsed inputs, identity assignment, graph context, manual asset and trust-boundary annotations, generated threat cards, passive identity comparison, an experiment notebook, runtime scope configuration and no-network preview, and a bounded active-replay panel. The Evidence view displays hash-linked records. Classroom now starts with 24 guided beginner lessons, including DevTools, Burp, evidence comparison, AI-assisted learning, and an optional GDK introduction. The optional reference syllabus retains 135 mapped topics: 15 short primers and 120 outlines. Start with the guided course; this manual supplies additional workflow and API detail.
...
The cheat sheet is a memory aid after you have completed Chapter 3. Here is what each command does and what success looks like:
...
A full free-form threat-diagram editor and automatic proxy integration are also not present. Classroom offers a 24-lesson guided beginner path. Its separate optional 135-topic syllabus contains 15 short primers and 120 outlines; these are not 135 complete courses. Use browser DevTools, Burp Suite, or Caido for authorized capture and manual proxy work, then use SurfaceTrace for structured investigation and evidence.
